...
### Экспериментальные способы встраивания
    ...
    y_sorted[:k * w], embedding_cost = stc_embed(x_sorted[:k * w], rho[order][:k * w], m, H_HAT, h)
    ...
    m = stc_extract(y_sorted, H_HAT, h)
    l = u32_from_bits(m[:32])
    ...
    plt.plot(range(len(rho_sorted)), np.log2(rho_sorted), marker=',', linestyle=' ')
    plt.show()